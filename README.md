...
Scryfall publishes several [bulk data files](https://scryfall.com/docs/api/bulk-data) updated daily. This project uses four of them:

| File | Used for |
|---|---|
| `oracle_cards` | Card names and Scryfall URIs |
...
`download-bulk-data.mjs` fetches those four files from the Scryfall API and saves them to `data/`. `build.mjs` reads them, cross-references art tags (animals) with oracle tags (actions), and produces:
